...
<?php late_image(get_source("single_threaded_gameloop.webp"),"","display: block; margin: auto; max-width: 100%;");?>
...
<source src="<?php echo get_source("multithreading_meme.mp4");?>" type="video/mp4">
</video>
...
<?php late_image(get_source("naive_system.webp"),"","display: block; margin: auto; max-width: 100%;");?>
...
<?php late_image(get_source("local_buffers.webp"),"","display: block; margin: auto; max-width: 100%;");?>
...
<?php late_image(get_source("steal_buffers.webp"),"","display: block; margin: auto; max-width: 100%;");?>
...
<?php late_image(get_source("deque.webp"),"","display: block; margin: auto; max-width: 100%;");?>
...
<p>To be able to invoke the function pointer, it is stored in an <span class="code">Option</span>, such that when calling it, we can take ownership of it. It should go without saying, that this <span class="code">Job</span> can only be invoked once, because once we moved the function pointer out of <span class="code">Option</span>, there is no function pointer left.</p>
...
<p>This code would be fine in a single threaded context, but this introduces a race condition. Between <span class="code">job_system::peak()</span> and <span class="code">job_system::pop()</span>, another thread may pop off a job. Thus, even though the if-statement may be <span class="code">true</span>, a different job is popped off than what was peaked at. This means, <span class="code">render_job</span> may actually not be a render job.</p>
...
<p><span class="code">is_empty()</span> may be useful, and I even have the use case for it, but ultimately it's redundant. Because <span class="code">steal()</span> and <span class="code">pop()</span> can simply return <span class="code">None</span> when no job exists, <span class="code">is_empty()</span> provides truly redundant information.</p>

<p><span class="code">count</span> or <span class="code">length</span> are also useless: Either there are jobs in the buffer that can be popped off, or there are none. There will never be a case, where I would want to know how many jobs are currently stored in the buffer.</p>
...
<?php late_image(get_source("friendship_ended.webp"),"","display: block; margin: auto; max-width: 100%;");?>

<p>My choice to implement the <span class="code">JobBuffer</span> fell onto the ring buffer. A ring buffer is just an array or vector, with one or two cursors, keeping track where the current head and tail are. It's called a ring buffer, because the cursors wrap around the array, thus it effectively has no start and end, like a ring. Once allocated, no further allocations are necessary to use it. Also, no nasty pointer issues arise. And to top it all off, a ring buffer can be entirely coded in safe Rust, which is always a big bonus.</p>
...
<p>The constructor should now be very straightforward. We create a vector with a fixed capacity, and insert empty nodes. Notice that we return an <span class="code">Arc&lt;Self&gt;</span>. This makes the client code a bit tidier, because this buffer is never used on its own, it will always be duplicated somehow.
...
<p>Push is a bit more complicated. Notice the unsafe block in the first line of this method. As I've explained earlier, this doesn't cause problems, because this will only ever be accessed by a single thread. We then lock the node at the current head. Notice how this is a <span class="code">try_lock</span>, not a <span class="code">lock</span>. This avoids blocking, meaning the job system can progress, even if the push failed. If we managed to lock the node, we can then see if there is a job inside or not. If there is a job already, the buffer is full. If there is no job, then we can overwrite it and update the head.</p>

<p>Note that the <span class="code">BlockedOrFull</span> error takes ownership of the job. This allows the not-pushed job to be returned to the caller, so that they may invoke it.</p>
...
<p>Pop is very similar, but the new head is computed first and then the node is locked. Note that it uses <span class="code">lock</span>, not <span class="code">try_lock</span>. A worker thread attempting to pop a job literally has nothing else to do. So to make progress, the best option is to block until we can access the node. Then, whether or not there is a job, we return the job or an <span class="code">IsEmpty</span> error.</p>
...
<p>Again, steal is very similar. But unlike the head in push and pop, the tail needs to be locked, because it is protected by a mutex. Notice how this one is using a <span class="code">try_lock</span> and not a <span class="code">lock</span>. This is due to there being <span class="code">number of CPUS - 1</span> amount of buffers to steal from. If we can't steal from this buffer, we simply steal from the next one. There is no need to block. After that, we then lock the node. If we were successful, we compute the next tail and return the job, otherwise we return an error.</p>
...
<p>This <span class="code">JobBuffer</span> is about 100 lines of code, but to test it I wrote 1180 lines! (including whitespaces)</p>
...
<p>But the painful, repetitive work was absolutely worth it. To this day, I haven't encountered a single issue with the <span class="code">JobBuffer</span>, and it's been <?php
...
<p>You may or may not have figured out, that the <span class="code">JobBuffer</span> described in the previous section is the local <span class="code">JobBuffer</span>, which is owned by a worker thread. What about the globally shared <span class="code">JobBuffer</span>? For better or for worse, I got rid of it.</p>
...
<p>First, we have a thread local variable. It stores an <span class="code">Option</span>, because a thread that wasn't initialized as a worker thread cannot serve as a worker thread.</p>

<p>The <span class="code">WorkerThread</span> currently stores 3 things: The <span class="code">JobBuffer</span> that it owns, the <span class="code">JobBuffer</span>s from the other workers, and an index, that uniquely identifies this thread. The index is mainly used for debug purposes, and it's a utility that directly comes with my job system. As such, the index isn't linked to the platform, the OS, or anything of that nature.</p>

<p><span class="code">JobSystemGuard</span> is the struct that will be returned when the job system is initialized. If this guard is dropped, the job system will be shut down. Due to its singleton-like behavior, it didn't feel right to have a struct that represents the job system, similar of how you would design an OOP singleton. Instead, it gives the caller the opportunity to drop it at a specified time. The <span class="code">JobSystemGuard</span> stores all handles of every spawned worker thread, and an <span class="code">AtomicBool</span> to signal when worker threads should be shut down.</p>
...
<p>The <span class="code">init</span> method may look daunting at first, but it's actually very straight forward. It takes 2 values: The size of the <span class="code">JobBuffer</span>s and how many threads should be spawned. First, we clamp the threads to the number of processors that your machine has. This is to prevent spawning more threads than cores.</p>
...
<p>After we've calculated the affinities, we simply create one <span class="code">JobBuffer</span> for every worker thread. We also create the <span class="code">done</span> flag.</p>

<p>Since the main thread is also a worker thread, we will be spawning <span class="code">threads - 1</span> number of workers. For each we clone the affinities, the buffers and the <span class="code">done</span> flag. Once spawned, each thread will first set itself up and then run itself.</p>

<p>Just right after that, the main thread is doing the same, but it also collects the handles and puts them into the <span class="code">JobSystemGuard</span>. The job system is now setup and already running!</p>
...
<p>First, we set the <span class="code">done</span> flag to <span class="code">true</span>, so every worker thread knows it's time to stop. The main thread then empties its local buffer, meaning it is popping all jobs left in the buffer and running them. Then we take the <span class="code">handles</span>, and join each worker thread. With some logging thrown in, that's all that <span class="code">drop</span> does.</p>
...
<p>This method duplicates the array of <span class="code">JobBuffer</span>s, thus every worker thread can have a copy of all available buffers.</p>
...
<p>Using 2 for loops, we then get the buffers from which will be stolen from. We start at <span class="code">index + 1</span> and then wrap around. This diagram illustrates how the buffers will be arranged, for worker thread 2 on a machine with 5 cores:</p>

<?php late_image(get_source("local_steal_buffers.webp"),"","display: block; margin: auto; max-width: 100%;");?>
...
<p>Running the worker thread is very straight forward. While the <span class="code">done</span> flag is <span class="code">false</span>, we are stuck in an endless loop, running jobs that are still waiting to be executed. If the <span class="code">done</span> flag is <span class="code">true</span>, we empty the buffer, like the main thread does when <span class="code">JobSystemGuard</span> is dropped.</p>
...
<p><span class="code">run_pending_job()</span> is public, because as you will see in the sections <i>JobFuture</i> and <i>JobCell</i>, it is quite useful to let others run jobs as well.</p>
...
<p>Popping literally does nothing except calling <span class="code">wait_and_pop()</span> of its local buffer. If for whatever reason this method is called from a non-worker thread, then this method returns nothing and prints an error instead.</p>
...
<p>Stealing is pretty much the same, except that because we have a vector of buffers, we iterate through them. The first steal that succeeds breaks the iteration. Just like <span class="code">pop_job()</span>, if this is called from a non-worker thread somehow, this method will print an error.</p>
...
<p>This simply returns the thread-local index. If the calling thread is not a worker thread, <span class="code">-1</span> is returned and an error is printed.</p>

<p>I reserved the submit function for last, because it uses <span class="code">JobFuture</span>s, which I'll explain in the next section. But here it finally is:</p>
...
<p>This function takes a closure, that can return a value. This is the job to be run. And it returns a <span class="code">JobFuture</span>, which can return the jobs returned value. You may remember, that <span class="code">Job</span> was a very very simple wrapper around a function pointer, <b>which doesn't even return anything</b>. That is intentional, to make my life easier. Instead of handing the closure directly into the job, and thus forcing me to somehow accommodate for its return value, it's much easier to simply wrap the closure into another closure, that doesn't return anything.</p>

<p><span class="code">submit()</span> first creates the future. This includes a <span class="code">SettableJobFuture</span> and a <span class="code">JobFuture</span>. Soon you will see, that they share a mutex, but the split between "settable" and "non-settable" prevents client code to set the future themselves.</p>

<p>Then it creates said closure, which sets the future with its return value. After that, we simply call <span class="code">push()</span> from our thread-local local buffer. Recall that pushing returns the job, when the buffer is full. In that case, we store it in <span class="code">not_pushed</span> and invoke it later. At the very end, we return the non-settable <span class="code">JobFuture</span>, so that the client may wait on the job that was just submitted.</p>
...
<p>This is what the <span class="code">JobFuture</span> is for. It gives client code a handle, which can be awaited. As you've already seen, it is split into a <span class="code">SettableJobFuture</span> and a non-settable <span class="code">JobFuture</span>. Let's just jump straight into it!</p>
...
<p><span class="code">Inner</span> is simply the data to be stored. It stores a bool <span class="code">is_ready</span>, which indicates whether the job is finished or not. <span class="code">data</span> is the value, which was returned by the job. It is stored inside an <span class="code">Option</span>, because sooner or later we need to move the data out. The Future can be in these 4 states:</p>
...
<p><span class="code">SettableJobFuture</span> and <span class="code">JobFuture</span> literally store the same: An <span class="code">Arc&lt;Mutex&lt;Inner&lt;T&gt;&gt;&gt;</span>. Thus, they are virtually identical. Because they are different structs however, they can implement different methods. For example, only <span class="code">SettableJobFuture</span> has a constructor, and it returns both a <span class="code">SettableJobFuture</span> and a <span class="code">JobFuture</span>. Also, only <span class="code">SettableJobFuture</span> can set its value. <span class="code">JobFuture</span> can wait on a value, but <span class="code">SettableJobFuture</span> cannot. Let's see:</p>
...
<p><span class="code">SettableJobFuture</span> is very straight forward. It initializes the <span class="code">Arc&lt;Mutex&lt;Inner&lt;T&gt;&gt;&gt;</span> which will be shared between the two structs. It then creates each struct and returns them. <span class="code">set()</span> simply locks the mutex, sets <span class="code">is_ready</span> to <span class="code">true</span>, and overwrites <span class="code">data</span>. Notice how <span class="code">set()</span> consumes <span class="code">self</span>. This directly means that once <span class="code">SettableJobFuture</span> is changed, it might as well be dead.</p>

<p>Let's look at the await-able <span class="code">JobFuture</span> next:</p>
...
<p>Just like <span class="code">set()</span> in <span class="code">SettableJobFuture</span>, <span class="code">wait()</span> also consumes self. This results in the fact, that taking <span class="code">data</span> will always succeed. We can't <span class="code">wait()</span> a second time, when it was awaited already. This is why the <span class="code">None</span> branch of <span class="code">wait()</span> includes an <span class="code">unreachable!()</span> and why <span class="code">wait()</span> can afford to return the value directly, instead of returning an <span class="code">Option</span>.</p>

<p><span class="code">wait()</span> calls <span class="code">wait_and_take()</span>, which spinlocks until <span class="code">data</span> is set. Considering the reactions I got from a <a href="https://www.rismosch.com/article?id=running-test-in-series" target="_blank" rel="noopener noreferrer">previous blogpost</a>, people somehow seem to hate spinlocks and immediately consider them unsafe. But let me assure you, that without a spinlock, this future wouldn't work.</p>

<p>Assume for a second, that <span class="code">wait()</span> uses a locking mechanism, like a condition variable or a mutex. In that case, <b>waiting for the future would block the entire thread</b>. This is problematic, because a blocked worker thread cannot run jobs. Ideally, the worker thread would do other things while the calling job is waiting. As such, a spinlock-like construct is the best choice for implementing such behavior.</p>

<p>This is what <span class="code">wait_and_take()</span> does: It first calls <span class="code">try_lock()</span> on the mutex, which isn't a blocking operation. If the lock was successful and the future was set, we can take the <span class="code">data</span> inside. If the lock was not successful, because it was poisoned, then this is unrecoverable and we throw an error. If the future was not set, or the lock resulted in a <span class="code">TryLockError::WouldBlock</span> error, then we run a pending job and attempt the whole procedure again.</p>

<p>In a nutshell, <span class="code">wait()</span> will be stuck in a loop, running pending jobs, as long as the job is not completed, thus effectively blocking the calling job and progressing the entire job system. With this taken care of, what else might be missing from this job system? Well...</p>
...
<?php late_image(get_source("borrow_checker_meme.webp"),"","display: block; margin: auto; max-width: 100%;");?>
...
<p>Before you say it, no, <span class="code">Arc&lt;Mutex&lt;T&gt;&gt;</span> does <b>NOT</b> work. Remember what I've written in the previous section <i>JobFuture</i>: If a mutex were to block, then the entire worker thread blocks, meaning no progress will be made. I could write a spinlock around <span class="code">try_lock()</span>, but this really isn't practical. Even if I were to create an elegant interface to spinlock a mutex, a spinlock still adds major overhead, which simply isn't worth it for just passing a single value into a job. This is especially true when I plan to spawn numerous jobs, all of which may reference their outside in one way or another.</p>

<p>What I want is an easy way to pass things into a job, with no overhead at all. Eventually, this led me to <span class="code">UnsafeCell</span>, and a much much deeper understanding of how Rusts ownership rules work.</p>
...
<p class="spoiler">No, this doesn't compile. That should come as no surprise. We are moving a value out of <span class="code">wrapper</span>! If you are accustomed to Rust, this should stick out like a sore thumb, and you would immediately realize that passing a <span class="code">&mut</span> into <span class="code">consume()</span> is probably what you wanted in the first place.</p>
...
<p class="spoiler">Yes, this does actually compile. This surprised me when I first saw this. We are still moving a value out of <span class="code">wrapper</span>, just like the first code snippet, so why does it compile? The reason why the first one didn't compile is not really because we moved something out. Rather, we tried to <i>use</i> a value which was already moved. If you remove the <span class="code">println!()</span> in the first example, we are not using <span class="code">wrapper</span> after its move and the code compiles just fine. This implies that it's A-okay to move child values out from a parent. We just gotta replace it before we use it again. Instead of replacing it with an arbitrary value, we can replace it with the value we moved out! Remember that the <span class="code">JobFuture</span> allows us to return something from a job. Any ownership that we may pass into a job we can return afterwards.</p>
...
<p>Ignore <span class="code">Ref</span> for a moment &#127313;. It most definitely is not the <span class="code">Ref</span> you are currently thinking about. We will talk about it in a second.</p>

<p>Notice how the <span class="code">run()</span> function takes ownership of an <span class="code">InputData</span> object &#127312;. It also returns an <span class="code">InputData</span> object &#127314;. This method alone is already demonstrating the technique. It takes ownership of <span class="code">InputData</span> and later returns it. The first thing we do is to pass ownership of <span class="code">current.keyboard</span> to a local variable &#127315;. Then we move the local variable and ownership into the job &#127316;. Now the job owns the keyboard and can mutate it however it wants. With an additional <span class="code">gameloop_state</span>, the job also returns the keyboard &#127317;, such that when the job is awaited, the caller once again has the ownership of <span class="code">keyboard</span> in a local variable &#127318;. To satisfy the compiler, we move this local variable <span class="code">new_keyboard</span> back into <span class="code">current.keyboard</span> &#127319;.</p>

<?php late_image(get_source("ownership_diagram.webp"),"","display: block; margin: auto; max-width: 100%;");?>

<p>No ownership rules have been broken, and yet a job was fully able to mutate the value. Notice how in the diagram above, no green "owns keyboard" blocks overlap on the y axis. At every single point in time, there exists exactly one owner of <span class="code">keyboard</span>, and as such the compiler is happy with this code.</p>

<br><br>

<p>This is cool, but why is this section called <span class="code">JobCell</span>? Apparently, we don't even need a <span class="code">Cell</span>-like structure to move things into a job! Up until this point I've talked about moving mutable values. But immutable references ain't so easy.</p>

<p>Immutable references are implicitly <span class="code">Copy</span>. That means that if a single immutable reference exists, you can make as many immutable references as you like. We may pass a single immutable reference into a job, but returning it is fruitless. The job may create God knows how many references from it. A single return can never accommodate for all references that have been created. To say it bluntly: <span class="code">&</span> is forbidden. No amount of ownership tricks can change this.</p>

<p>So we need a <span class="code">Cell</span>-like struct afterall, that keeps track of all references somehow. Let me introduce my <span class="code">JobCell</span>:</p>

<?php late_image(get_source("jobcell.webp"),"","display: block; margin: auto; max-width: 100%;");?>

<p>This <span class="code">JobCell</span> can be in two states: Either simply <span class="code">JobCell</span>, which allows you to make as many immutable references as you like. Or <span class="code">MutableJobCell</span>, which allows you to mutate its content. To switch between the two states, some extra logic is required:</p>

<p>To switch from <span class="code">JobCell</span> to <span class="code">MutableJobCell</span>, all immutable references must be dead. To do this, we spinlock and call pending jobs as long as immutable references exist. This already imposes a restriction: Immutable references, created by the <span class="code">JobCell</span>, are only allowed to be passed into a Job. If you were to create an immutable reference in the same thread where you want to mutate data, or attempt to store the reference somewhere indefinitely, then switching to the mutable state will <i>livelock</i>. A livelock is similar to a deadlock, except it is using up resources by spinning, instead of just going to sleep like a deadlock.</p>

<p>But if this restriction is taken care of, the spinlock ends and a <span class="code">MutableJobCell</span> is created. To switch back to <span class="code">JobCell</span>, all you have to do is to drop <span class="code">MutableJobCell</span>. Now here comes a trick: Since <span class="code">MutableJobCell</span> borrows <span class="code">JobCell</span> mutably, the Rust compiler forbids you to use <span class="code">JobCell</span> while <span class="code">MutableJobCell</span> lives. This means as long as the <span class="code">JobCell</span> is in its mutable state, no immutable references can be created. Not via <span class="code">JobCell</span>s interface, nor via Rusts default reference mechanics.</p>
...
<p>Here we have all structs to make the magic work. <span class="code">JobCell</span> is a simple wrapper around <span class="code">UnsafeCell</span>, but it additionally keeps count of its current references via an <span class="code">AtomicUsize</span>. As mentioned previously, <span class="code">MutableJobCell</span> stores a <i>reference</i> to an <span class="code">UnsafeCell</span>, instead of straight up owning it. This directly leads to the mechanism, that <span class="code">JobCell</span> cannot be used while <span class="code">MutableJobCell</span> lives. Finally, we need to mimic <span class="code">&</span>, by wrapping a pointer into a struct called <span class="code">Ref</span>. Since this wrapper is its own thing, and syntactically independent from <span class="code">JobCell</span>, the compiler thinks that client code owns it. To ensure that <span class="code">Ref</span> mimics an immutable reference, only <span class="code">Deref</span> will be implemented on it. It also doesn't implement any methods that could mutate its state in any way. Additionally, <span class="code">Ref</span> stores a counter of all alive references. This counter is increased when <span class="code">Ref</span> is being cloned, or decreased when <span class="code">Ref</span> is being dropped. And because of some compiler optimization aliasing safety shenanigans tic-tac-toe i-don't-know-what-i-am-talking-about, <span class="code">Ref</span> must also store a <span class="code">PhantomData</span>.</p>
...
<p>Like I have said a thousand times already, the constructor should be fairly self-explanatory. It creates the <span class="code">UnsafeCell</span> and the reference counter.</p>

<p>Since we went over the theory already, <span class="code">as_mut()</span> should also be fairly straight forward. In a spinlock it constantly checks whether any references exist, and it runs pending jobs if they do. Only when no references exist, a <span class="code">MutableJobCell</span> is created and returned.</p>

<p>If you've worked with Rusts pointers before, <span class="code">borrow()</span> shouldn't be that difficult to understand either. But before we do any pointer magic, we increase the reference count by 1, indicating that from now on, an immutable reference exists. Then, we create a pointer to the data of the <span class="code">UnsafeCell</span>. This value is never modified, or dereferenced mutably, so this is safe. Then we simply construct the <span class="code">Ref</span> struct and return it.</p>

<p>To make my life easier, <span class="code">MutableJobCell</span> implements one additional function:</p>
...
<p>This is a simple wrapper to replace the inner value of the <span class="code">JobCell</span>.</p>
...
<p><span class="code">Deref</span> and <span class="code">DerefMut</span> for <span class="code">MutableJobCell</span> are quite obvious, because we want to access and mutate the underlying data.</p>
...
<p><span class="code">Deref</span> for <span class="code">Ref</span> is also a no brainer. Very important: <span class="code">Ref</span> does NOT implement <span class="code">DerefMut</span>, otherwise this would be undefined behavior galore!</p>
...
<p>To clone a <span class="code">Ref</span>, we simply increase the reference counter and return a new <span class="code">Ref</span> with the same data.</p>
...
<p>Dropping a <span class="code">Ref</span> decreases the reference counter. We don't need to free the pointer, because <span class="code">Ref</span> doesn't semantically own it.</p>

<p>And that is basically it! To see the <span class="code">JobCell</span> in action, here's some excerpt from my god job / main game loop:</p>
...
<p>Every buffer exists 2 times: Once as owned and once as <span class="code">JobCell</span>. It’s pretty much a front- and back-buffer system. The jobs that own a buffer, can modify them as much as they want. The jobs that only borrow the <span class="code">JobCell</span> cannot mutate the buffer, and can only create new state which follows from the previous state.</p>

<p>Each iteration of the loop does the same: First, it switches each <span class="code">JobCell</span> to <span class="code">MutableJobCell</span> by calling <span class="code">as_mut()</span> on them. If any immutable references exist at this point, these calls will spinlock and run all leftover jobs. Then the owned buffer and the <span class="code">JobCell</span> buffer will be swapped. After that, all necessary <span class="code">Ref</span>s are created. Then these references are passed into the according jobs. Because SDL2 can only pump window events on the thread that spawned the window (which is my main thread), the input frame is not put into a job. And since we need to await it anyway, we might as well call it synchronously. After each job is done, we simply move ownership back, and another iteration of the loop is ready to be run.</p>
...
<video loop="true" autoplay="autoplay" muted="true" style="max-width:100%; display: block; margin: auto;" loading='lazy'>
<source src="<?php echo get_source("100_cpu_usage.mp4");?>" type="video/mp4">
</video>
